  ...
  class AwsEcsWatcher < BaseWatcher
    ...
    def start
      ...
      if @discovery['aws_access_key_id'] && @discovery['aws_secret_access_key']
        @ec2 = Aws::EC2::Client.new(region: region, access_key_id: @discovery['aws_access_key_id'], secret_access_key: @discovery['aws_secret_access_key'] )
        @ecs = Aws::ECS::Client.new(region: region, access_key_id: @discovery['aws_access_key_id'], secret_access_key: @discovery['aws_secret_access_key'] )
      else
      ...
      unless @discovery['method'] == 'aws_ecs'
        ...
        unless @discovery['aws_ecs_cluster']
          ...
          def configure_backends(new_backends)
            ...
            def api_task_ids
              #api_cluster_ids
              resp = @ecs.list_tasks(({cluster: @discovery['aws_ecs_cluster'], family: @discovery['aws_ecs_family']})).map(&:task_arns)
              #log.info "---------------- \n\nCalled list_services #{resp.data.task_arns}\n\n\n"
              return resp
            end
